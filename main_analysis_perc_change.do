...
foreach k of local groups {
    ...
    * Get xb at 2024 (reference year) — no offset, matches pred_visits
    quietly sum year if agegroup == "`k'" & year == 2024
    
    * Use predictnl to get SE of log-ratio between years
    * log(pred_yr / pred_2024) = xb_yr - xb_2024  (offset cancels)
    
    * First get xb at 2024 as a fixed scalar
    tempvar xb_all
    ...
    * predictnl: SE of (xb_yr - xb_2024) for 2030 and 2035
    ...
    * % change = 100*(exp(d) - 1), CI on log scale then exponentiate
    replace pct_2024_2030_est = 100*(exp(`d30') - 1) ///
        if agegroup == "`k'" & year == 2030
    replace pct_2024_2030_lb  = 100*(exp(`d30' - 1.96*`s30') - 1) ///
        if agegroup == "`k'" & year == 2030
    replace pct_2024_2030_ub  = 100*(exp(`d30' + 1.96*`s30') - 1) ///
        if agegroup == "`k'" & year == 2030

    replace pct_2024_2035_est = 100*(exp(`d35') - 1) ///
        if agegroup == "`k'" & year == 2035
    replace pct_2024_2035_lb  = 100*(exp(`d35' - 1.96*`s35') - 1) ///
        if agegroup == "`k'" & year == 2035
    replace pct_2024_2035_ub  = 100*(exp(`d35' + 1.96*`s35') - 1) ///
        if agegroup == "`k'" & year == 2035

    drop s1 s2 s3
}

* ── Collapse to one row per group for display (matches your original) ─
* Propagate to all rows within group for the keep step
...
* ── 95% CI for observed % change 2022→2024 (raw counts) ─────────────
* Uses Poisson exact CI for a count ratio
* log(n2024/n2022) ± 1.96*sqrt(1/n2024 + 1/n2022)
* This is the standard asymptotic CI for a ratio of two Poisson counts
...
foreach k of local groups {
    ...
    * Log ratio and its SE (Poisson variance = 1/n for log counts)
...
foreach k of local groups {
    ...
    local e : display %5.1f r(mean)
    quietly sum pct_obs_lb if agegroup == "`k'"
    local l : display %5.1f r(mean)
    quietly sum pct_obs_ub if agegroup == "`k'"
    local u : display %5.1f r(mean)
    di "`k'   |  `e'% (`l' to `u')"
}
